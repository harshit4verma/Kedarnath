...
class ControllerManager {
  ...
  initTouchLook() {
    ...
    window.addEventListener('mousedown', (e) => {
      if (e.target.closest('button, a, input, select, textarea, .real-360-card, .modal-content, .nandi-card, .bottom-nandi-dock')) return;
    ...
    window.addEventListener('mouseup', () => { this.isDraggingLook = false; });

    // Smooth Zoom FOV with mouse wheel (30° - 95°)
  ...
  update(delta, time) {
    if (this.mode === 'real360') {
      ...
      // Gentle panoramic drift when not actively dragging
      if (!this.isDraggingLook) {
        this.euler.y -= delta * 0.03;
        this.camera.quaternion.setFromEuler(this.euler);
      }
