...
class KedarnathApp {
  ...
  initUI() {
    ...
    // 5b. Real 360° Video VR Experience Mode Toggle
  ...
  // =========================================================================
  // AUTHENTIC 360° REAL PHOTOSPHERE VR VIEWER
  ...
  initReal360Viewer() {
    ...
    const configTex = (tex) => {
      ...
      if (this.real360Mat) this.real360Mat.needsUpdate = true;
    };

    this.real360Textures = {
      day: texLoader.load('assets/kedarnath_360_day_courtyard.jpg', configTex),
      close: texLoader.load('assets/kedarnath_360_temple_close.jpg', configTex),
      bhim: texLoader.load('assets/kedarnath_360_bhim_shila.jpg', configTex),
      night: texLoader.load('assets/kedarnath_360_aarti_night.jpg', configTex)
    };
  ...
  exitReal360Mode() {
    ...
    // Immediately mute/stop any Aarti audio playing inside 360 degree mode
    if (window.soundEngine) {
      window.soundEngine.pauseRealAarti();
      window.soundEngine.stopRealAarti();
    }

    // Restore pre-360 environment time of day (e.g. day or user selection)
    const restoreTime = this.savedTimeOfDay || 'day';
    this.environment.setTimeOfDay(restoreTime);
    this.temple.setTimeOfDay(restoreTime);
    ...
    // Reset 360 Aarti audio button state
  ...
  switch360View(viewKey) {
    ...
    if (viewKey === 'day') {
      ...
      if (descEl) descEl.textContent = 'Authentic 360° VR courtyard darshan with pilgrims and Himalayan peaks';
